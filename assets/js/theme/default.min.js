...
$("#menu-layer").load("menu.html");

function cargaHtml( rutaArchivo ){
	if( rutaArchivo!='') {
		//this.parent.className = 'active';
		/*
		if( current_active!==null ) current_active.className = '';
		current_active = this.parent;
		console.log(current_active);
		current_active.className = 'active';
		*/
		$("#content").load(rutaArchivo);
	}
}

function showNextModal(idModalLayer)
{
	setTimeout(
		function(){
			var textElem = '<a href="#'+idModalLayer+'" data-toggle="modal" id="a-'+idModalLayer+'"></a>';
			document.body.innerHTML += textElem;
			$("#a-"+idModalLayer).click();
			window.document.getElementById("a-"+idModalLayer).parentElement.removeChild(window.document.getElementById("a-"+idModalLayer));
		},
		1000
	);
}

function forceRedirect(htmlPage)
{
	setTimeout(
		function(){ window.location.href = htmlPage+'.html';},
		500
	);
}

function removeItem(idElem)
{
	window.document.getElementById(idElem).parentElement.removeChild(window.document.getElementById(idElem));
}